...
const MODAL_CONTENT: Record<NonNullable<ModalType>, { eyebrow: string; heading: string; sub: string }> = {
  midway: {
    ...
    sub:      'Reserve your return ride now for priority service.',
  },
  pre_dropoff: {
    eyebrow:  'Almost There',
    heading:  'Ready for your next ride?',
    sub:      'Book before drop-off and lock in your rate.',
  },
};
    ...
    <AnimatePresence>
      {type && content && (
        <motion.div
          ...
          style={{ background: 'rgba(6,6,10,0.85)', backdropFilter: 'blur(12px)' }}
          onClick={onClose}
        >
          <motion.div
            ...
            style={{ background: '#0F0F14', border: '1px solid rgba(201,168,76,0.14)' }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-lux-muted hover:text-lux-white transition-colors text-base"
            >
              ✕
            </button>
            ...
            <p className="text-[13px] text-lux-muted mb-8">{content.sub}</p>
            ...
            <p className="text-center text-[11px] text-lux-muted mt-4 tracking-wider">
              OR CALL&nbsp;&nbsp;
