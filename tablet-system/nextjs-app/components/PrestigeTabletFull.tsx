...
type RideState = "start" | "arrival" | "cruise" | "mid-ride" | "pre-dropoff";
type DataStatus = "live" | "stale" | "error";
type ConfirmationStage = "confirming" | "confirmed" | "done";
type ComfortLevel = "cool" | "normal" | "warm";
    ...
    <div style={{ position: "relative", zIndex: 1, padding: "44px 48px", minHeight: 560, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.9, ease: EASE, delay: 0.15 } }} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <label style={{ fontSize: 11, letterSpacing: "1.5px", textTransform: "uppercase", color: `${CREAM}0.35)`, fontFamily: SANS, fontWeight: 300 }}>Client Name</label>
          <input
            value={name}
            onChange={(e) => { setName(e.target.value); setError(false); }}
            placeholder="e.g. James Torres"
            style={{ background: `${GOLD}0.06)`, border: `0.5px solid ${error && !name.trim() ? "rgba(200,80,80,0.5)" : `${GOLD}0.25)`}`, borderRadius: 8, padding: "14px 16px", color: "#f0ece4", fontSize: 15, fontFamily: SANS, fontWeight: 300, outline: "none", width: "100%" }}
          />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <label style={{ fontSize: 11, letterSpacing: "1.5px", textTransform: "uppercase", color: `${CREAM}0.35)`, fontFamily: SANS, fontWeight: 300 }}>Destination</label>
          <input
            value={destination}
            onChange={(e) => { setDestination(e.target.value); setError(false); }}
...
function StatusBanner({ status }: { status: DataStatus }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.7, ease: EASE } }} exit={{ opacity: 0, y: -8, transition: { duration: 0.5, ease: EASE } }} style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: 8, padding: "7px 16px", background: "rgba(15,13,11,0.85)", border: `0.5px solid ${isError ? "rgba(200,80,80,0.2)" : `${GOLD}0.2)`}`, borderRadius: 20, backdropFilter: "blur(12px)", zIndex: 30 }}>
      <div style={{ width: 5, height: 5, borderRadius: "50%", background: isError ? "rgba(200,80,80,0.7)" : `${GOLD}0.6)` }} />
      <span style={{ fontSize: 11, letterSpacing: "1.5px", textTransform: "uppercase", color: `${CREAM}0.4)`, fontWeight: 300, fontFamily: SANS }}>{isError ? "Route data unavailable" : "Updating route..."}</span>
    </motion.div>
  );
}
...
// ── Quick Action Modal ────────────────────────────────────────────────────────
function ActionModal({ type, onClose, onComfortChange, comfortLevel }: { type: "request" | "comfort" | "receipt" | null; onClose: () => void; onComfortChange: (level: ComfortLevel) => void; comfortLevel: ComfortLevel; }) {
  if (!type) return null;
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 0.4, ease: EASE } }} exit={{ opacity: 0, transition: { duration: 0.3, ease: EASE } }} style={{ position: "absolute", inset: 0, background: "rgba(10,10,10,0.92)", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", padding: 48 }} onClick={onClose}>
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1, transition: { duration: 0.6, ease: EASE } }} onClick={(e) => e.stopPropagation()} style={{ background: "rgba(22,20,18,0.98)", border: `0.5px solid ${GOLD}0.2)`, borderRadius: 16, padding: "32px 36px", width: "100%", maxWidth: 400 }}>
        {type === "request" && (
          <>
            <h3 style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 300, color: "#f0ece4", marginBottom: 8 }}>Driver Request</h3>
            <p style={{ fontSize: 13, color: `${CREAM}0.4)`, fontFamily: SANS, fontWeight: 300, marginBottom: 24, lineHeight: 1.6 }}>Mr. Rodriguez will be notified immediately.</p>
            {["Adjust temperature", "Pull over briefly", "Change route", "Need assistance"].map((req) => (
              <button key={req} onClick={onClose} style={{ display: "block", width: "100%", padding: "13px 16px", background: `${GOLD}0.06)`, border: `0.5px solid ${GOLD}0.2)`, borderRadius: 8, color: `${CREAM}0.7)`, fontSize: 13, fontFamily: SANS, fontWeight: 300, cursor: "pointer", textAlign: "left", marginBottom: 8 }}>{req}</button>
            ))}
          </>
        )}
        {type === "comfort" && (
          <>
            <h3 style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 300, color: "#f0ece4", marginBottom: 8 }}>Cabin Comfort</h3>
            <p style={{ fontSize: 13, color: `${CREAM}0.4)`, fontFamily: SANS, fontWeight: 300, marginBottom: 24 }}>Set your preferred temperature.</p>
            <div style={{ display: "flex", gap: 10 }}>
              {(["cool", "normal", "warm"] as ComfortLevel[]).map((level) => (
                <button key={level} onClick={() => { onComfortChange(level); onClose(); }} style={{ flex: 1, padding: "14px 8px", background: comfortLevel === level ? `${GOLD}0.2)` : `${GOLD}0.06)`, border: `0.5px solid ${comfortLevel === level ? `${GOLD}0.6)` : `${GOLD}0.2)`}`, borderRadius: 8, color: comfortLevel === level ? "#d4b896" : `${CREAM}0.5)`, fontSize: 13, fontFamily: SANS, cursor: "pointer", textTransform: "capitalize" }}>{level}</button>
              ))}
            </div>
          </>
        )}
        {type === "receipt" && (
          <>
            <h3 style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 300, color: "#f0ece4", marginBottom: 8 }}>Your Receipt</h3>
            <p style={{ fontSize: 13, color: `${CREAM}0.4)`, fontFamily: SANS, fontWeight: 300, marginBottom: 24 }}>A receipt will be sent to your email on file.</p>
            <button onClick={onClose} style={{ width: "100%", padding: "14px", background: `${GOLD}0.12)`, border: `0.5px solid ${GOLD}0.4)`, borderRadius: 8, color: "#d4b896", fontSize: 13, fontFamily: SANS, fontWeight: 400, letterSpacing: "0.8px", cursor: "pointer", textTransform: "uppercase" }}>Send Receipt</button>
          </>
        )}
        <button onClick={onClose} style={{ marginTop: 16, width: "100%", padding: "12px", background: "transparent", border: `0.5px solid ${CREAM}0.08)`, borderRadius: 8, color: `${CREAM}0.25)`, fontSize: 12, fontFamily: SANS, cursor: "pointer" }}>Dismiss</button>
      </motion.div>
    </motion.div>
  );
}
...
function PassiveView({ rideState, dataStatus, clientName, destination, comfortLevel, onActionTap, vipRides, externalEta }: {
  ...
  const greetingText = rideState === "arrival"
    ...
    : rideState === "pre-dropoff"
    ? "Almost there."
    : `Enjoy the\nride.`;

  const comfortLabel = { cool: "68°F · Cool", normal: "72°F · Comfort", warm: "76°F · Warm" }[comfortLevel];
  const ridesNeeded = Math.max(0, 10 - (vipRides % 10));
  const vipProgress = ((vipRides % 10) / 10) * 100;

  const actions = [
    { key: "request" as const, icon: "◎", label: "Request", sub: "Driver assistance" },
    { key: "comfort" as const, icon: "❄", label: "Comfort", sub: comfortLabel },
    { key: "receipt" as const, icon: "◻", label: "Receipt", sub: "View charges" },
  ];
  ...
  return (
    <div style={{ position: "relative", zIndex: 1, padding: "44px 48px 40px", minHeight: 560, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {rideState === "cruise" && (
          <motion.div key="actions" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.9, ease: EASE, delay: 0.24 } }} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {actions.map((action) => (
              <button key={action.key} onClick={() => onActionTap(action.key)} style={{ padding: "16px 12px", background: `${GOLD}0.06)`, border: `0.5px solid ${GOLD}0.2)`, borderRadius: 10, cursor: "pointer", textAlign: "center", display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>
                <span style={{ fontSize: 18, color: `${GOLD}0.7)` }}>{action.icon}</span>
                <span style={{ fontSize: 12, fontWeight: 400, color: `${CREAM}0.8)`, fontFamily: SANS, letterSpacing: "0.5px", textTransform: "uppercase" }}>{action.label}</span>
                <span style={{ fontSize: 10, fontWeight: 300, color: `${CREAM}0.35)`, fontFamily: SANS }}>{action.sub}</span>
              </button>
            ))}
          </motion.div>
        )}

        {rideState === "cruise" && vipRides > 0 && (
          <motion.div key="vip" initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 0.9, ease: EASE, delay: 0.36 } }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={{ fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", color: `${GOLD}0.5)`, fontFamily: SANS, fontWeight: 300 }}>Prestige Status</span>
              <span style={{ fontSize: 10, color: `${CREAM}0.3)`, fontFamily: SANS, fontWeight: 300 }}>{ridesNeeded} ride{ridesNeeded !== 1 ? "s" : ""} to Gold</span>
            </div>
            <div style={{ height: 2, background: `${GOLD}0.1)`, borderRadius: 2, overflow: "hidden" }}>
              <motion.div initial={{ width: 0 }} animate={{ width: `${vipProgress}%`, transition: { duration: 1.2, ease: EASE, delay: 0.5 } }} style={{ height: "100%", background: `${GOLD}0.6)`, borderRadius: 2 }} />
            </div>
          </motion.div>
        )}
      </div>
...
function MidRidePrompt({ onBook, onDismiss }: { onBook: () => void; onDismiss: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1, transition: { duration: 1.0, ease: EASE } }} exit={{ opacity: 0, y: 20, transition: { duration: 0.6, ease: EASE } }} style={{ position: "absolute", bottom: 40, left: 48, right: 48, background: "rgba(22,20,18,0.95)", border: `0.5px solid ${GOLD}0.25)`, borderRadius: 14, padding: "24px 28px", backdropFilter: "blur(20px)", zIndex: 10 }}>
...
export default function PrestigeTabletFull({
  ...
  return (
    <div style={{ fontFamily: SANS, background: "#0a0a0a", color: "#f0ece4", minHeight: 560, width: "100%", position: "relative", overflow: "hidden", borderRadius: 16 }}>
      ...
      <AnimatePresence mode="wait">
        {isStartScreen ? (
          <motion.div key="start" initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 1.0, ease: EASE } }} exit={{ opacity: 0, transition: { duration: 0.7, ease: EASE } }}>
            <RideStartScreen onStart={handleStart} />
          </motion.div>
        ) : showFallback ? (
